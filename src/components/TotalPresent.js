export default function TotalPresent(props) {
    return (
        <div className="flex flex-col flex-grow basis-64 w-full bg-primary rounded-lg p-3">
            ...
            <div className="text-5xl pl-3 flex flex-row items-baseline text-accent">{props.present ? props.present : '0'}<h2 className="flex flex-row text-base">{`/ ${props.present ? props.present : '0'} present`}</h2></div>
